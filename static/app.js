...
const convertBtn = $("#convert-btn"), scrubBtn = $("#scrub-btn"), countEl = $("#count");
...
async function addFiles(fileList){
  ...
  setStatus(`Generating previews for ${images.length} image(s)…`);
...
async function convertAll(mode){
  ...
  convertBtn.disabled = true; progress.hidden = false;
  ...
  try {
    if (outs.length === 1 && !errors.length){
...
$("#clear-btn").addEventListener("click", () => { state.items = []; render(); setStatus(""); });
...
// Frameless native-window title-bar controls. In a plain browser (self-host / fallback) these are inert.
document.querySelectorAll(".titlebar .wbtn").forEach((b)=>b.addEventListener("click",()=>{
  const api = window.pywebview && window.pywebview.api; if (!api) return;
  const a = b.dataset.win;
  if (a === "min") api.minimize();
  else if (a === "max") api.toggle_maximize();
